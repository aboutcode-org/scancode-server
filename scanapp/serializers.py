...
class CopyrightSerializer(serializers.ModelSerializer):
    """
    ModelSerializer for `Copyright` with all fields excluding `id` and
    `scanned_file`
    """
...
class CopyrightHolderSerializer(serializers.ModelSerializer):
    """
    ModelSerializer for `CopyrightHolders` with all fields excluding `id`
    and `copyright`
    """
...
class CopyrightStatementSerializer(serializers.ModelSerializer):
    """
    ModelSerializer for `CopyrightHolders` with all fields excluding `id` and
    `copyright`
    """
...
class CopyrightAuthorSerializer(serializers.ModelSerializer):
    """
    ModelSerializer for `CopyrightAuthor` with all fields excluding `id` and
    `copyright`
    """
...
class PackageSerializer(serializers.ModelSerializer):
    """
    ModelSerializer for `Package` with all fields excluding `id` and
    `scanned_file`
    """
...
class AllModelSerializer(serializers.Serializer):
    """
    A single serializer that takes care of all the Model Serializers and bind
    them together
    """
    scan = ScanSerializer()
...
class AllModelSerializerHelper(object):
    ...
    def __init__(self, scan):
        ...
        self.license = License.objects.filter(
            scanned_file__in=(self.scanned_file))
        self.copyright = Copyright.objects.filter(
            scanned_file__in=(self.scanned_file))
        self.copyright_holder = CopyrightHolder.objects.filter(
            copyright__in=(self.copyright))
        self.copyright_statement = CopyrightStatement.objects.filter(
            copyright__in=(self.copyright))
        self.copyright_author = CopyrightAuthor.objects.filter(
            copyright__in=(self.copyright))
        self.package = Package.objects.filter(
            scanned_file__in=(self.scanned_file))
        self.scan_error = ScanError.objects.filter(
            scanned_file__in=(self.scanned_file))
